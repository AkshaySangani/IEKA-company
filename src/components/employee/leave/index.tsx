...
export const initialEmployeeLeaveRequest: IEmployeeLeaveRequest = {
  _id: "",
  userId: {
    _id: "",
    firstName: "",
...
const EmployeeLeaveRequest: React.FC = () => {
  ...
  // get branch list
  const fetchEmployeeLeaveRequestList = async (payload: {
    page: number;
    limit: number;
    search: string;
    ...
    if (response.success && response.data?.leaves?.length > 0) {
      setEmployeeLeaveRequestList(response.data?.leaves);
      setTotal(response.data?.total);
    } else {
      setEmployeeLeaveRequestList([]);
      setTotal(0);
      setPage(1);
    }
    setLoading(false);
