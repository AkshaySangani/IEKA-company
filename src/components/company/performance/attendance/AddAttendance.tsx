...
import { addAttendance, AttendanceFormData, getAttendanceById, updateAttendance } from "../../../../apis/performance/attendance.api";
import { getBranchAndShift } from "../../../../apis/organization/department.api";
...
export interface IAttendanceBranch {
  _id: string;
  name: string;
  address: string;
}

export interface IAttendanceAssignment {
  _id: string;
  branchId: IAttendanceBranch;
  shiftIds: IAttendanceShift[];
}
...
export interface IAttendanceResponse {
  ...
  companyId: string;
  name: string;
  assignments: IAttendanceAssignment[];
  status: statusEnum;
  createdAt: string;
  updatedAt: string;
}
...
const AddAttendance: React.FC = () => {
  ...
  useEffect(() => {
    if (departmentId) {
      fetchAttendanceById(departmentId);
    }
  }, [departmentId]);

  // get department by id
  const fetchAttendanceById = async (departmentId: string) => {
    setLoading(true);
    const response = await getAttendanceById(departmentId);
    if (response.success) {
      const department: IAttendanceResponse = response.data;
  ...
  const scrollToFirstError = (
    errors: Partial<Record<keyof AttendanceFormData, string>>,
  ) => {
          ...
          <div className="mt-8 border-t border-borderPrimary pt-6">
            <div className="mb-5 flex items-center gap-2">
              ...
              <h3 className="text-sm font-medium">
                Select Branch & Shift to Assign this Attendance
              </h3>
            </div>
